...
/**
 * Resolves a connectionId from the connection-related parameters that can be passed
 * to SDK methods (connectionId, connection, authenticationId).
 *
 * Handles mutual exclusivity validation, string alias lookup via the connections map,
 * and numeric passthrough.
 */
export async function resolveConnectionId({ connectionId, connection, authenticationId, resolveConnection, label, }) {
    const hasDirectId = connectionId != null || authenticationId != null;
    if (hasDirectId && connection != null) {
        const context = label ? ` for ${label}` : "";
        throw new ZapierValidationError(`Cannot specify both connectionId/authenticationId and connection${context}. Use connectionId for a direct ID, or connection for an alias or numeric ID.`);
    }
    let resolvedConnectionId = connectionId ?? authenticationId ?? undefined;
    if (connection != null && resolvedConnectionId == null) {
        if (typeof connection === "number") {
            resolvedConnectionId = connection;
        }
        else {
            // Try the connections map first, then fall back to direct ID for
            // numeric strings and UUIDs.
            ...
            if (entry) {
                ...
                throw new ZapierConfigurationError(`Connection "${connection}" not found in connections map. ` +
                    "Ensure the connection is defined in the manifest connections, or pass a connection ID directly.", { configType: "connections" });
            }
        }
    }
