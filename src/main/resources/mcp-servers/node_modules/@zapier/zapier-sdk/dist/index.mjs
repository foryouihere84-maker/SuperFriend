...
// src/resolvers/clientCredentialsName.ts
var clientCredentialsNameResolver = {
  type: "static",
  inputType: "text",
    ...
    const response = await api.get(
      "/api/v0/client-credentials",
      {
        ...
        customErrorHandler: ({ status }) => {
          if (status === 401) {
            return new ZapierAuthenticationError(
              `Authentication failed. Your token may not have permission to access client credentials or may be expired. (HTTP ${status})`,
              { statusCode: status }
            );
          }
          if (status === 403) {
            return new ZapierAuthenticationError(
              `Access forbidden. Your token may not have the required scopes to list client credentials. (HTTP ${status})`,
              { statusCode: status }
            );
          }
          return void 0;
...
var createClientCredentialsPlugin = ({ context }) => {
    ...
    const response = await api.post(
      ...
      },
      {
        customErrorHandler: ({ status }) => {
          if (status === 401) {
            return new ZapierAuthenticationError(
              `Authentication failed. Your token may not have permission to create client credentials or may be expired. (HTTP ${status})`,
              { statusCode: status }
            );
          }
          if (status === 403) {
            return new ZapierAuthenticationError(
              `Access forbidden. Your token may not have the required scopes to create client credentials. (HTTP ${status})`,
              { statusCode: status }
            );
          }
          return void 0;
  ...
  const createClientCredentialsDefinition = createFunction(
...
var deleteClientCredentialsPlugin = ({ context }) => {
  ...
  const deleteClientCredentialsDefinition = createFunction(
