...
/**
 * Complex 模式聊天策略
 * 带 Plan-Execute 的复杂任务对话
 * 与 Lite 模式接口风格一致，唯一区别是 ReAct/Skills/MCP + Plan-Execute
 */
@Slf4j
@Component
public class ComplexChatStrategy implements ChatModeStrategy {
    ...
    public void executeChat(AIChatRequest request, UserIntent intent,
        ...
        log.debug("Complex 模式执行普通对话: sessionId={}", request.getSessionId());
        // 使用统一接口，与 Lite 模式风格一致
        mcpHostService.chatWithMCPComplex(request, intent, callback);
    }

    @Override
    public void executeMultimodalChat(AIChatRequest request, UserIntent intent,
                                       Consumer<AIChatResponse> callback) {
        log.debug("Complex 模式执行多模态对话: sessionId={}", request.getSessionId());
        // 使用统一接口，与 Lite 模式风格一致
        mcpHostService.chatWithMCPComplexMultimodal(request, intent, callback);
    }
    ...
    public void executeFileParsedChat(AIChatRequest request, UserIntent intent,
                                       ...
                                       Consumer<AIChatResponse> callback) {
        log.debug("Complex 模式执行文件解析后对话: sessionId={}", request.getSessionId());
        ...
        // 使用统一接口
        mcpHostService.chatWithMCPComplex(enhancedRequest, intent, callback);
    }
