...
export const extractChatSession = async (
  ...
  const response = await post<{ success: boolean; status: string; message: string }>(
    `/api/v16/chat/extract/${sessionId}`,
...
export const getExtractionStatus = async (
  ...
  sessionId: string
  status: 'NONE' | 'PENDING' | 'EXTRACTING' | 'COMPLETED'
  message: string
  inProgress: boolean
  ...
  const response = await get<{
    ...
    inProgress: boolean
    remainingSeconds?: number
  }>(`/api/v16/chat/extraction-status/${sessionId}`)
  return response
}

// 强制重新提取知识
export const forceFinalizeChatSession = async (
  sessionId: string,
  userId?: number,
  model?: string
): Promise<{ success: boolean; message: string }> => {
  const response = await post<{ success: boolean; message: string }>(
    `/api/v16/chat/finalize/${sessionId}/force-extraction`,
    { userId, model }
  )
  return response
}
